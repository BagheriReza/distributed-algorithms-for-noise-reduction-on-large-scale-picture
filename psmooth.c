...
unsigned char  window [9],//for window buffer
...
for(j=0;j<(gap+2);j++)//repeat untill of each process size finish
{
if(rank==0){
...
window [8]=array3[z] ;
	...
	result[z]  = (window[0] + window[3] + window[6] +
	      window[1] + window[4] + window[7] +
	      window[2] + window[5] + window[8])/9 ;//calculate sum of window buffer
} 
...
window [8]=array3[z] ;
	...
	q = window[0] + window[3] + window[6] +
	      window[1] + window[4] + window[7] +
	      window[2] + window[5] + window[8] ;//calculate sum of window buffer
	
	result[z] = q / 9;// caculate filter value for this pixel
